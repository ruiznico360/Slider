package mobile.slider.app.slider.ui.Calculator.BD;
...
"230924783917373501229842420499568935992206602204654941510613");
                ...
                else
                {
                        if ( q.isIntegerFrac() )
                        {
                                ...
                                BigDecimal reserr = new BigDecimal( 0.5* q.abs().doubleValue()
                                        * x.ulp().divide(x.abs(),MathContext.DECIMAL128).doubleValue() ) ;
                                ...
                                for( ;; )
                                {
                                        ...
                                        Util.log(nu + " " + de);
                                        BigDecimal eps = nu.divide(de,MathContext.DECIMAL128) ;

                                        BigDecimal err = res.multiply(reserr,MathContext.DECIMAL128) ;
                                        int precDiv = 2+err2prec(eps,err) ;
                                        if ( precDiv <= 0 )
                                        {
                                                /* The case when the precision is already reached and any precision
                                                 * will do. */
                                                eps = nu.divide(de,MathContext.DECIMAL128) ;
                                        }
                                        else
                                        {
                        ...
                        else
                        {
                                 ...
                                 */
                                int precq =  3+err2prec( (x.ulp().divide(x,MathContext.DECIMAL128)).doubleValue()
        ...
        * @author Richard J. Mathar
        */
        static public BigDecimal powRoundOriginal(final BigDecimal x, final Rational q)
        {
                /** Special cases: x^1=x and x^0 = 1
                */
                if ( q.compareTo(BigInteger.ONE) == 0 )
